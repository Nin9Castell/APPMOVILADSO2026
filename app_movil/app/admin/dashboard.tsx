 ...
 * incluye accesos rapidos y la secciones mas usadas
 ...
 * las versiones actuales del iOS y Android no permiten uso de push
 *
 */
...
type StatCard = {
    ...
    gradient: [string, string]; //par de colores fondo principal y secundario
    route: string; // ruta a la q se navega al precionar la tarjeta
    show: boolean; // si es false la tarjeta no se muestra
};
// componente principal del dashboard
export default function adminDashboardScreen() {
    // contexto de autenticacion
    // se usa cast a tipo explicito porque AuthContext.js es js puro y no reconoce TSX
    ...
    const [stats, setStats] = useState({
        ...
        subcategorias: 0,
        productos: 0,
        usuarios: 0, //numero de usuarios registrados solo el admon
        pedidos: 0,
        pendientes: 0,
        ventas: 0,
    });
    ...
    useEffect(() => {
        const load = async () => {
            setLoading(true);
            try {
                // se hacen 4 peticiones en paralelo con primise all para reducir el tiempo de carga
                // cada un trae datos de una seccion diferente al panel
                ...
                // estraer los datos de cada respuesta con opcional chaining y allback
                const catsData = cats.data?.data?.categorias || [];
                const subsData = subs.data?.data?.subcategoria || [];
                ...
                // si algina peticion falla simplemente se ignora el error
                // los stats quedan en 0 no se muestra el mensaje de error al usuario
                // ignore
            } finally {
                setLoading(false);
            }
        ...
        /**
         * solo carga las estadisticas si el usuario esta autenticado y tiene rol de admin o auxilar
         */
        if (isAuthenticated && (isAdmin || isAux)) load();
    }, [isAuthenticated, isAdmin, isAux]); // se ejecuta segun el rol q inicio sesion 

    // si el usuario no esta autenticado o no tiene rol administrador o auxiliar muestra bloqueo de acceso
