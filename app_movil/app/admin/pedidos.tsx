 ...
 * permite buscar pedidos por txt en tiempo rial mientras escribe 
...
export default function AdminPedidoScreen() {
    ...
    const [pagina, setPaginas] = useState(1); //pagian actual es la 1
    ...
    const fetchPedidos = async (page = 1, search = '') => {
        ...
        setErrorMessage('');
        try {
            //construye el query del string dinamicamnte
            const params: string[] = [];
            if (search.trim()) params.push(`buscar=${encodeURIComponent(search.trim())}`); //aspdkasoidja
