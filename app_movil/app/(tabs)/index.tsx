...
export default function HomeScreen() {
     ...
     * como dependencias se ejecuta una sola vez al montr el componente
     */
     ...
     * cuando el usuario escribe en el buscador o cambia de cateogria
    ...
    //Productos filtrados subconjunto de productos que coninciden con la busqueda y categoria
    const productosFiltrados = useMemo (() => {
        const termino = busqueda.trim().toLowerCase(); //normaliza el texto
        return productos.filter((p : any) => {
            //coincideTexto el producto tiene el termino en su nomb
            const coincideTexto =
            termino === '' ||
            ...
            //categoriaId o cateogria.id son los 2 campos posibles segun el endpoint
            const coincideCategoria =
            categoriaActiva === 'all' ||
            //pasa todo
            String(p.cateogriaId || p.cateogria?.id) === categoriaActiva;
            return coincideTexto && coincideCategoria;
        });
    }, [busqueda, categoriaActiva, productos]); //recalcula cuando cambia la busqueda categoria o datos

    /**
     * hasproductos: true si hay al menos un producto filtrado evita mostrar la vista vacia
     */
    ...
    productosFiltrados.slice((paginaActual -1) * ITEMS_POR_PAGINA, paginaActual * ITEMS_POR_PAGINA), [productosFiltrados, paginaActual, ITEMS_POR_PAGINA]);
